...
def main():
    ...
    while n < 30000:
        data_set = 'Set_A.dat'
        listPts = readDataPts(data_set, n)  #File name, numPts given as example only
        giftwrap_dict[n] =  t.timeit(giftwrap(listPts))
        listPts = readDataPts(data_set, n) #You may replace these three print statements
        graham_dict[n] = grahamscan(listPts) #with any code for validating your outputs
        listPts = readDataPts(data_set, n)
        quick_hull_dict[n] = amethod(listPts)
        n += 2000

    print(quick_hull_dict)
